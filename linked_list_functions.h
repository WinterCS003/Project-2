...
node<ITEM_TYPE>* SearchList(node<ITEM_TYPE>* head,  //IN - list head
                            ITEM_TYPE key){         //IN - val to search for
    for(node<ITEM_TYPE>* current = head; current != nullptr ;current = current->next){
...
node<ITEM_TYPE>* _InsertHead(node<ITEM_TYPE> *&head,     //IN - list head
                             ITEM_TYPE insertThis){      //IN - val to insert
    head = new node<ITEM_TYPE>(insertThis);
}
...
node<ITEM_TYPE>* _InsertBefore(node<ITEM_TYPE>*& head,       //IN - list head
    ...
    for(node<ITEM_TYPE>* current = head; current->next != nullptr; current = current->next){
        if(current->next == beforeThis){
            current->next = new node<ITEM_TYPE>*(insertThis);
            current->next->next = beforeThis;
        }
    }
}
...
ITEM_TYPE _DeleteNode(node<ITEM_TYPE>*&head,            // IN - list head
    ...
    for(node<ITEM_TYPE>* current = head; current->next != nullptr; current = current->next){
        if(current->next == deleteThis){
            node<ITEM_TYPE>* temp = current->next;
            current->next = temp->next;
            ITEM_TYPE i = temp->_item;
            delete temp;
...
template <typename ITEM_TYPE>
node<ITEM_TYPE>* _CopyList(node<ITEM_TYPE>* head){ // IN - list head
    node<ITEM_TYPE>* copyHead = new node<ITEM_TYPE>;

    for(node<ITEM_TYPE> *current = head, *current2 = copyHead; current != nullptr; current = current->next, current2 = current2->next){
        current2 = new node<ITEM_TYPE>(current->_item);
...
template <typename ITEM_TYPE>
void _ClearList(node<ITEM_TYPE>*& head){ // IN - list head
    for(node<ITEM_TYPE>* current = head; current != nullptr;){
        node<ITEM_TYPE>* temp = current;
...
ITEM_TYPE& At(node<ITEM_TYPE>* head,        // IN - list head
    ...
    int i = 0;
    for(node<ITEM_TYPE>* current = head; current != nullptr; current = current->next, i++){
        if(i == pos){
...
template <typename ITEM_TYPE>
int _find(node<ITEM_TYPE>* head, ITEM_TYPE key){
    int i = 0;
    for(node<ITEM_TYPE>* current = head; current != nullptr; current = current->next, i++){
        if(current->_item == key){
            return i;
